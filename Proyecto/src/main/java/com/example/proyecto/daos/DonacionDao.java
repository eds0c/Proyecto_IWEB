...
public class DonacionDao extends DaoBase{
    ...
    public int cantidadDonacionesPorEstado(String estado){


        ArrayList<Donacion> lista = new ArrayList<>();
        int cantidad=0;

        String sql = "select count(idDonacion) from donacion where estado = ?;";
    ...
    public int cantidadDeDonacion(String fecha){
    ...
    public int sumaDeDonacion(String fecha){
        ...
        String sql = "select sum(monto), fecha from donacion where fecha = ? and estado = 'aprobado' group by fecha;";
