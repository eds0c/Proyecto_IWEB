                    ...
                    <div class="filter-buttons" id="buttons">
                        ...
                        <div class="slider">
                            ...
                            <a class="button-value text-decoration-none" role="button"
                               href="<%=request.getContextPath()%>/AlumnoServlet?idAct=<%=a.getIdActividad()%>"><%=a.getTitulo()%>
                            </a>
                            <%
                                };
                            %>
                ...
                <div class="col">
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            ...
                            <li class="page-item">
                                <a class="page-link" href="<%=request.getContextPath() %>/AlumnoServlet?action=main_page&currentPage=<%=auxStart%>&idAct=<%=idActividadJsp%>" aria-label="Previous">
                                    <span aria-hidden="true">&laquo;</span>
                                </a>
                            </li>
                            <%for (int j=1;j<=cantidadPaginas;j++){%>
                            <%if(currentPageJsp == j){%>
                            <li class="page-item"><a class="page-link active" href="<%=request.getContextPath() %>/AlumnoServlet?action=main_page&currentPage=<%=j%>&idAct=<%=idActividadJsp%>"><%=j%></a></li>
                            <%} else {%>
                            <li class="page-item"><a class="page-link" href="<%=request.getContextPath() %>/AlumnoServlet?action=main_page&currentPage=<%=j%>&idAct=<%=idActividadJsp%>"><%=j%></a></li>
                            <%}%>
                            <%}%>
                            <li class="page-item">
                                <a class="page-link" href="<%=request.getContextPath() %>/AlumnoServlet?action=main_page&currentPage=<%=auxEnd%>&idAct=<%=idActividadJsp%>" aria-label="Next">
